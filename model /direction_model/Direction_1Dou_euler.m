function [angle_acf,t]=OU1D_euler( alpha, mu, sigma, v0, tmax, n_steps, weight )
...
%
  ...
  angle = zeros ( length(alpha), n_steps + 1 );

  angle(:,1) = v0;

for i = 1:length(alpha)
    for j = 1:n_steps
        angle(i, j+1) = angle(i, j) + dt * alpha(i) * (mu(i) - angle(i, j)) + sigma(i) * dw(i, j);
    end
end
angle_acf = sum(angle .* sqrt(weight'), 1);
