function [x,t]=euler_maruyama ( num_comp, x0,y0, T, dt , weight, alpha, omega,sigma)

%*****************************************************************************
% Applies the Euler-Maruyama method to the Ornstein-Uhlenbeck SDE.
...
%Integration cycle
    for j = 1:n_steps
       X(:,j+1) = X(:,j) -  alpha .* X(:,j) * dt -  omega .* Y(:,j) * dt + sigma * sqrt ( dt ) .* randn ( num_comp,1 );
