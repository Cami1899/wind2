...
% 	%make sure input is valid
	assert(size(P_0_root,1)==state_count &&size(P_0_root,2)==state_count);
	assert(size(C,1)==sensor_count && size(C,2)==state_count);
	assert(size(Q_root,1)==state_count &&size(Q_root,2)==state_count);
	assert(size(x_0,1)==state_count && size(x_0,2)==1);
	assert(size(R_root,1)==sensor_count &&size(R_root,2)==sensor_count);
	...
	for k=1:measurement_count
		...
		%and update phase
		[x_k_m,P_root_km] = cdekf_predict_phase(f_func,jacobian_func,h,dt_between_measurements,current_time,P_root_km1_p,x_km1_p,Q_root);

[h_xp,y,J_h,R]=observation_function(x,t_val,t,dt,t_G,t_B,t_wind,J_h1_num,J_h2_num,J_h3_num,J_h4_num,J_h5_num,J_h6_num,J_h7_num,ac_mG,ac_mB,ar_mG,ar_mB,windG,ac_variance,err_wind,ar_variance)


		[x_k_p,P_root_kp] = cdekf_update_phase(R_root,P_root_km,C,x_k_m,measurements(:,k));
